...
async def quest1_8(message: types.Message):
    await bot.send_message(text="Ви маєте право звернутися до суду для визнання права власності на майно, яке не визнається чи оспорюється.", chat_id=message.from_user.id)
...
async def quest2_3(message: types.Message):
    await bot.send_message(text="Здійснити виклик Державної служби з надзвичайних ситуацій (зателефонувати за номером «101») для роЗМІнування, усунення наслідків та складання акту про пожежу та акту пошкодження або руйнування майна із зазначенням причини такого руйнування.", chat_id=message.from_user.id)
...
async def quest2_4(message: types.Message):
    await bot.send_message(text="До такого акту вносяться відомості, зокрема, про дату, час, місце, опис знищеного та пошкодженого майна, прямі та побічні збитки, причину тощо.", chat_id=message.from_user.id)
...
async def quest4_2(message: types.Message):
    await bot.send_message(text="Для того, щоб зафіксувати інформацію про кримінальне правопорушення. На підставі заяви громадянина вноситься інформація в Єдиний реєстр досудових розслідувань (ЄРДР) за фактом знищення або пошкодження майна внаслідок воєнних дій.", chat_id=message.from_user.id)
...
async def quest4_3(message: types.Message):
    await bot.send_message(text="Копію протоколу та витяг з Єдиного реєстру досудових розслідувань", chat_id=message.from_user.id)
...
async def quest4_4(message: types.Message):
    await bot.send_message(text="Подати заяву в поліцію можна не тільки у випадку пошкодження майна внаслідок бойових дій, а також у випадках встановлення факту мародерства, пограбування або коли об’єктом пошкодження є транспортний засіб", chat_id=message.from_user.id)
...
async def quest5_3(message: types.Message):
    await bot.send_message(text="Збитки, які можуть виникнути на землі та ґрунті внаслідок надзвичайної ситуації та/або збройної агресії, можуть включати:\n"
                                ...
                                "5. Інші збитки, пов'язані з псуванням або втратою використовуваної земельної ділянки.", chat_id=message.from_user.id)
...
async def quest5_5(message: types.Message):
    await bot.send_message(text="Пошкодження або забруднення земельної ділянки, водних об’єктів", chat_id=message.from_user.id)
...
async def quest5_6(message: types.Message):
    await bot.send_message(text="Так. Електронна пошта eco@shtab.gov.ua", chat_id=message.from_user.id)
...
async def quest6_2(message: types.Message):
    await bot.send_message(text="Так. Рекомендації можна знайти за посиланням (Алгоритм дій з фіксації)", chat_id=message.from_user.id)


@dp.message_handler(text="Чи достатньо відео фіксації чи фото фіксації?")
async def quest6_3(message: types.Message):
    await bot.send_message(text="Ні. Потрібно також зібрати необхідні документи. Детальніше за посиланням (Алгоритм дій з фіксації)", chat_id=message.from_user.id)
...
async def quest6_11(message: types.Message):
    await bot.send_message(text="Для зменшення ризику втрати зібраних матеріалів, рекомендовано зберігати їх на декількох пристроях та у хмарних сховищах", chat_id=message.from_user.id)
...
async def quest6_13(message: types.Message):
    await bot.send_message(text="Збережіть зібрані матеріали на декількох пристроях та у хмарних сховищах", chat_id=message.from_user.id)


@dp.message_handler(text="Чи є зразок акту, який необхідно скласти самостійно?")
async def quest6_14(message: types.Message):
    await bot.send_message(text="Акт складається в довільній формі. Зразок форми можна знайти тут (посиланням на Алгоритм дій з фіксації)", chat_id=message.from_user.id)
...
async def quest7_1(message: types.Message):
    await bot.send_message(text="Власники майна, посадові особи сільської /селищної/міської ради, працівники правоохоронних органів", chat_id=message.from_user.id)
...
async def quest7_3_2(message: types.Message):
    await bot.send_message(text="Опис обставини пошкодження житла, характеру пошкоджень, майна, яке знаходилось в будинку, дата, час та місце його складення, паспортні дані, місце проживання та контактну інформацію осіб, що підписують такий документ", chat_id=message.from_user.id)
...
async def quest8_2(message: types.Message):
    await bot.send_message(text="Необхідно намагатися зберегти посилання на такі інформаційні ресурси (скріншоти веб-сторінок, збереження відеозаписів, фото, зафіксувати попередньо дату та час доступу до інформаційного ресурсу, адресу ресурсу)", chat_id=message.from_user.id)
...
async def quest8_4(message: types.Message):
    await bot.send_message(text="Статті. Публікації. Скріншоти веб-сторінок", chat_id=message.from_user.id)


@dp.message_handler(text="Як можна зберегти опубліковані матеріали?")
async def quest8_5(message: types.Message):
    await bot.send_message(text="Скріншоти вебсторінок, збереження відеозаписів, фото, зафіксувати попередньо дату та час доступу до інформаційного ресурсу, адресу ресурсу", chat_id=message.from_user.id)
...
async def quest9_1(message: types.Message):
    await bot.send_message(text="Якщо відсутні чеки або інші документи, які підтверджують приналежність майна потерпілому та його вартість, можна спробувати скористатися такими доказами:\n"
                                ...
                                "3. Свідчення свідків: Якщо є свідки, які можуть підтвердити приналежність майна потерпілому або його вартість, їхні свідчення можуть бути використані як доказ.", chat_id=message.from_user.id)
...
async def quest9_2(message: types.Message):
    await bot.send_message(text="1. Супутніми доводами можуть виступати квитанції на рухоме майно (техніку, речі та інше, що знаходилось в будинку), фото майна до та після руйнування.\n"
                                "2. Також необхідно зібрати за наявності будь-які чеки (квитанції, фактури) на пошкоджене чи знищене майно, яке знаходилося в нерухомості, зробити з них копії.", chat_id=message.from_user.id)
...
async def quest9_5(message: types.Message):
    await bot.send_message(text="1. В деяких випадках електронні копії чеків можуть бути прийняті як доказ при заяві про пошкодження або знищення майна, особливо якщо вони мають дату, підпис або печатку магазину.\n"
                                "2. Проте, в інших випадках можуть бути встановлені певні вимоги щодо збереження оригінальних чеків або їх паперових копій.", chat_id=message.from_user.id)
...
async def quest10_3_4(message: types.Message):
    await bot.send_message(text="Якщо документи про підключення комунальних послуг були втрачені, їх можна отримати наступними способами:\n"
                                ...
                                "3. Звернутися до органів місцевого самоврядування або відповідних відділів з питань комунальних послуг і отримати необхідну інформацію та документи.", chat_id=message.from_user.id)


@dp.message_handler(text="Які комунальні послуги потрібно зазначати в акті?")
async def quest10_5(message: types.Message):
    await bot.send_message(text="Газопостачання, електропостачання, водопостачання, водовідведення, теплопостачання", chat_id=message.from_user.id)
...
async def quest11_2(message: types.Message):
    await bot.send_message(text="Потрібно сканувати та зберігати всі зібрані матеріали, включаючи документи, фотографії, акти, листи тощо", chat_id=message.from_user.id)


@dp.message_handler(text="На яких пристроях зберігати скани документів?")
async def quest11_3(message: types.Message):
    await bot.send_message(text="Скани документів можна зберігати на різних пристроях, таких як комп'ютери, ноутбуки, смартфони, планшети, зовнішні накопичувачі або спеціальні пристрої для зберігання документів.", chat_id=message.from_user.id)
...
async def quest11_4(message: types.Message):
    await bot.send_message(text="Для зберігання сканів документів в електронному вигляді можна використовувати різні хмарні сховища, такі як Google Drive, Dropbox, Microsoft OneDrive, iCloud тощо. Вибір конкретного хмарного сховища залежатиме від ваших вподобань та вимог щодо безпеки та доступності.", chat_id=message.from_user.id)
