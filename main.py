...
@dp.callback_query_handler(text="🏠📄Підтвердження права власності на майнo")
...
async def quest1_1(message: types.Message):
    await bot.send_message(text="✅  Договір, за яким відповідно до законодавства передбачається перехід права власності, зокрема купівлі-продажу, міни, дарування, довічного утримання, лізингу, предметом якого є нерухоме майно, про припинення права на аліменти для дитини у зв'язку з передачею права власності на нерухоме майно, договір іпотеки, що містить застереження про задоволення вимог іпотекодержателя, договір про задоволення вимог іпотекодержателя, спадковий договір (за наявності свідоцтва органу реєстрації актів цивільного стану про смерть чи рішення суду про оголошення особи померлою), договір про виділ у натурі частки з нерухомого майна, що є у спільній власності, про поділ нерухомого майна, що є у спільній власності.\n"
                                ...
                                "✅  Дублікат правовстановлювального документа, виданий нотаріусом, органом місцевого самоврядування, органом приватизації, копія архівного правовстановлювального документа, видана державним архівом.", chat_id=message.from_user.id)
...
async def quest1_7(message: types.Message):
    await bot.send_message(text="📝👨‍💼Слід звернутися із письмовою заявою до органу місцевого самоврядування чи нотаріуса для отримання дублікату.", chat_id=message.from_user.id)
...
async def quest1_8(message: types.Message):
    await bot.send_message(text="⚖️Ви маєте право звернутися до суду для визнання права власності на майно, яке не визнається чи оспорюється.", chat_id=message.from_user.id)
...
async def quest1_9(message: types.Message):
    await bot.send_message(text="✅Так", chat_id=message.from_user.id)


@dp.message_handler(text="👩‍🚒Фіксація ДСНС")
@dp.callback_query_handler(text="👩‍🚒Фіксація ДСНС")
...
async def quest2_1and2(message: types.Message):
    await bot.send_message(text="📷🏚Фіксацію фактів руйнування/пошкодження об'єктів нерухомого майна бажано здійснювати у максимально короткий час після вчинення акту агресії рф, що спричинив його руйнацію/пошкодження з урахуванням безпекової ситуації.", chat_id=message.from_user.id)


@dp.message_handler(text="Що робити у випадку пожежі або аварійної ситуації?")
async def quest2_3(message: types.Message):
    await bot.send_message(text="👩‍🚒Здійснити виклик Державної служби з надзвичайних ситуацій (зателефонувати за номером «101») \n⚠️для розмінування, \n⚠️усунення наслідків \n📋складання акту про пожежу та акту пошкодження або руйнування майна із зазначенням причини такого руйнування.", chat_id=message.from_user.id)
...
async def quest2_4(message: types.Message):
    await bot.send_message(text="До такого акту вносяться відомості, зокрема, \n📆про дату, \n🕤час, \n🏘місце, \n📝🏚опис знищеного та пошкодженого майна, прямі та побічні збитки, причину тощо.", chat_id=message.from_user.id)


@dp.message_handler(text="Як отримати копії актів про пожежу або пошкодження майна?")
async def quest2_5(message: types.Message):
    await bot.send_message(text="Якщо не отримано 📄акт про пошкодження на місці, тоді вам потрібно звернутися до місцевого підрозділу ДСНС🏫👩‍🚒", chat_id=message.from_user.id)
...
@dp.callback_query_handler(text="👨‍💼Фіксація сільською/селищною міською радою, у разі їх відсутності військовою адміністрацією або військово-цивільною адміністрацією")
...
async def quests3_1_2_3(message: types.Message):
    await bot.send_message(text="📝Зразкову форму можна знайти за посиланням (Алгоритм дій з фіксації, Додаток 3)", chat_id=message.from_user.id)
...
async def quest3_4(message: types.Message):
    await bot.send_message(text="💁‍особисто – шляхом звернення до загального відділу відповідної ради з заявою \n📨поштою – шляхом направлення заяви поштовим відправленням (краще направляти цінним листом з описом вкладення).", chat_id=message.from_user.id)
...
async def quest3_5_6(message: types.Message):
    await bot.send_message(text="📝 заява на проведення обстеження житла;\n"
                                ...
                                "📑🏠 копій документів, що підтверджують наявність права власності на житло на момент подання заяви.", chat_id=message.from_user.id)


@dp.message_handler(text="Що робити, якщо відсутні сільська /селищна/міська ради?")
async def quest3_7(message: types.Message):
    await bot.send_message(text="💁‍Варто звернутися до військово-цивільної адміністрації або іншої компетентної установи на території вашого населеного пункту", chat_id=message.from_user.id)


@dp.message_handler(text="Чи можна отримати копії актів про пошкодження майна?")
async def quest3_8(message: types.Message):
    await bot.send_message(text="✅Так, якщо не отримано акт про пошкодження на місці, тоді вам треба звернутися до міської або селищної ради, або військово-цивільної адміністрації.", chat_id=message.from_user.id)


@dp.message_handler(text="👮Фіксація НПУ")
@dp.callback_query_handler(text="👮Фіксація НПУ")
...
async def quest4_2(message: types.Message):
    await bot.send_message(text="✅Для того, щоб зафіксувати інформацію про кримінальне правопорушення. \n📑На підставі заяви громадянина вноситься інформація в Єдиний реєстр досудових розслідувань (ЄРДР) за фактом знищення або пошкодження майна внаслідок воєнних дій.", chat_id=message.from_user.id)
...
async def quest4_3(message: types.Message):
    await bot.send_message(text="📑Копію протоколу \n📑витяг з Єдиного реєстру досудових розслідувань", chat_id=message.from_user.id)
...
async def quest4_4(message: types.Message):
    await bot.send_message(text="📝Подати заяву в поліцію можна не тільки у випадку: \n🏚пошкодження майна внаслідок бойових дій, а також у випадках: \n🔓встановлення факту мародерства, \n🚷пограбування \n🚘Або коли об’єктом пошкодження є транспортний засіб", chat_id=message.from_user.id)
...
async def quest4_5(message: types.Message):
    await bot.send_message(text="📝👮‍Звернутись із заявою до територіального підрозділу у вашому регіоні", chat_id=message.from_user.id)
...
async def quest4_6(message: types.Message):
    await bot.send_message(text="✅Так, для фіксації скоєння кримінального правопорушення з подальшим внесенням відомостей до ЄРДР", chat_id=message.from_user.id)
...
@dp.callback_query_handler(text="♻Фіксація державною екологічною інспекцією")
...
async def quest5_1(message: types.Message):
    await bot.send_message(text="Для зафіксування завданої шкоди довкіллю, земельній ділянці, водним об'єктам ви можете виконати наступні кроки: \n📷📹 Зробіть фотографії або відеозаписи, які чітко демонструють пошкодження або знищення. \n📍🗺 Відзначте місце події на мапі або визначте його географічні координати. \n📆🕗 Запишіть дату та час виникнення події, а також будь-які інші важливі деталі, що стосуються завданої шкоди.", chat_id=message.from_user.id)
...
async def quest5_3(message: types.Message):
    await bot.send_message(text="Збитки, які можуть виникнути на землі та ґрунті внаслідок надзвичайної ситуації та/або збройної агресії, можуть включати:\n"
                                "⚠️ Забруднення ґрунту хімічними речовинами або іншими токсичними речовинами.\n"
                                "⚠️ Пошкодження верхнього шару ґрунту або його ерозія.\n"
                                "⚠️ Втрата родючості ґрунту та його негативний вплив на сільськогосподарські угіддя.\n"
                                "⚠️ Втрата розсіяних посівів, знищення сільськогосподарської техніки або будівель.\n"
                                "⚠️ Інші збитки, пов'язані з псуванням або втратою використовуваної земельної ділянки.", chat_id=message.from_user.id)
...
async def quest5_4(message: types.Message):
    await bot.send_message(text="Щоб зателефонувати на гарячу лінію Оперативного штабу при Державній екологічній інспекції України, ви можете скористатися наступними кроками:\n"
                                ...
                                "👨‍💼Зачекайте на відповідь оператора гарячої лінії.", chat_id=message.from_user.id)
...
async def quest5_5(message: types.Message):
    await bot.send_message(text="Пошкодження або забруднення \n⚠️земельної ділянки, \n⚠️водних об’єктів, \n⚠️пошкодження ґрунтового покриву тощо.", chat_id=message.from_user.id)
...
async def quest5_6(message: types.Message):
    await bot.send_message(text="✅Так. \n📧Електронна пошта eco@shtab.gov.ua", chat_id=message.from_user.id)


@dp.message_handler(text="🤳Самостійна фіксація")
@dp.callback_query_handler(text="🤳Самостійна фіксація")
...
async def quest6_1(message: types.Message):
    await bot.send_message(text="📱Телефон, 📷фото- чи 📹відеокамера", chat_id=message.from_user.id)


@dp.message_handler(text="Чи є рекомендації щодо порядку фіксації ?")
async def quest6_2(message: types.Message):
    await bot.send_message(text="✅Так. \n📒Рекомендації можна знайти за посиланням (Алгоритм дій з фіксації)", chat_id=message.from_user.id)


@dp.message_handler(text="Чи достатньо відео фіксації чи фото фіксації?")
async def quest6_3(message: types.Message):
    await bot.send_message(text="❌Ні. \n❗️Потрібно також зібрати необхідні документи. \n📒Детальніше за посиланням (Алгоритм дій з фіксації)", chat_id=message.from_user.id)
...
async def quest6_4(message: types.Message):
    await bot.send_message(text="📒Рекомендації можна знайти за посиланням (Алгоритм дій з фіксації)", chat_id=message.from_user.id)


@dp.message_handler(text="Як правильно здійснити фотофіксацію завданих збитків?")
async def quest6_5(message: types.Message):
    await bot.send_message(text="📒Порядок дій за посиланням (Алгоритм дій з фіксації)", chat_id=message.from_user.id)
...
async def quest6_9(message: types.Message):
    await bot.send_message(text="📍Геолокація дозволить в майбутньому об'єктивно перевірити та підтвердити місце зйомки", chat_id=message.from_user.id)
...
async def quest6_10(message: types.Message):
    await bot.send_message(text="✅Це допоможе в майбутньому підтвердити факт руйнування/пошкодження саме внаслідок бойових дій та отримати компенсацію", chat_id=message.from_user.id)


@dp.message_handler(text="Де потрібно зберігати зібрані матеріали з фіксації?")
async def quest6_11(message: types.Message):
    await bot.send_message(text="Для зменшення ризику втрати зібраних матеріалів, рекомендовано зберігати їх \n📱💻🖥на декількох пристроях \n📨у хмарних сховищах", chat_id=message.from_user.id)
...
async def quest6_12(message: types.Message):
    await bot.send_message(text="✅Так, потрібно описати та відобразити у відеофайлі  характер пошкодження або знищення : яке пошкодження, в якому приміщенні або частині будинку", chat_id=message.from_user.id)


@dp.message_handler(text="Що робити з усіма зібраними матеріалами?")
async def quest6_13(message: types.Message):
    await bot.send_message(text="Збережіть зібрані матеріали на  \n📱💻🖥 декількох пристроях \n📨у хмарних сховищах", chat_id=message.from_user.id)


@dp.message_handler(text="Чи є зразок акту, який необхідно скласти самостійно?")
async def quest6_14(message: types.Message):
    await bot.send_message(text="📝Акт складається в довільній формі. \n📄Зразок форми можна знайти тут (посиланням на Алгоритм дій з фіксації)", chat_id=message.from_user.id)


@dp.message_handler(text="👥Збір пояснень свідків")
@dp.callback_query_handler(text="👥Збір пояснень свідків")
...
async def quest7_1(message: types.Message):
    await bot.send_message(text="🙋‍Власники майна; \n👨‍💼посадові особи сільської /селищної/міської ради; \n👮‍працівники правоохоронних органів", chat_id=message.from_user.id)
...
async def quest7_3_2(message: types.Message):
    await bot.send_message(text="📝Опис обставини пошкодження житла; \n🏚характер пошкоджень, майна, яке знаходилось в будинку; \n📆дата; \n🕗час; \n📍місце його складення; \n🪪паспортні дані; \n🏘місце проживання; \n🗂контактну інформацію осіб, що підписують такий документ", chat_id=message.from_user.id)
...
async def quest7_7(message: types.Message):
    await bot.send_message(text="📝Власноручно написану заяву, в якій будуть відображені повні відомості про свідка:\n"
                                "🪪Прізвище, ім'я, по батькові;\n"
                                "🏘Адреса проживання;\n"
                                "📱📧 Всі наявні засоби зв’язку;\n"
                                "📄 Детальне викладенням обставин події, безпосереднім свідком яких стала особа;\n"
                                "✍️Письмове пояснення свідку необхідно підписати власноруч.", chat_id=message.from_user.id)


@dp.message_handler(text="🔎Збір матеріалів зі ЗМІ")
@dp.callback_query_handler(text="🔎Збір матеріалів зі ЗМІ")
...
async def quest8_2(message: types.Message):
    await bot.send_message(text="✅Необхідно намагатися зберегти посилання на такі інформаційні ресурси, наприклад:\n"
                                "✅ Скріншоти веб-сторінок;\n"
                                "✅Збереження відеозаписів, фото, зафіксувавши попередньо дату та час доступу до інформаційного ресурсу;\n"
                                "✅Адреса ресурсу.", chat_id=message.from_user.id)


@dp.message_handler(text="Чому важливо зберігати публікації ЗМІ?")
async def quest8_3(message: types.Message):
    await bot.send_message(text="✅Такі матеріали стануть додатковим аргументом на підтвердження факту руйнування/пошкодження майна", chat_id=message.from_user.id)


@dp.message_handler(text="Які матеріали від ЗМІ слід зберегти?")
async def quest8_4(message: types.Message):
    await bot.send_message(text="✅Статті. \n✅Публікації. \n✅Скріншоти веб-сторінок", chat_id=message.from_user.id)


@dp.message_handler(text="Як можна зберегти опубліковані матеріали?")
async def quest8_5(message: types.Message):
    await bot.send_message(text="Скріншоти вебсторінок, збереження відеозаписів, фото, зафіксувати попередньо дату та час доступу до інформаційного ресурсу, адресу ресурсу✅Необхідно намагатися зберегти посилання на такі інформаційні ресурси, наприклад:\n"
                                "✅Скріншоти веб-сторінок;\n"
                                "✅ Збереження відеозаписів, фото, зафіксувавши попередньо дату та час доступу до інформаційного ресурсу;\n"
                                "✅Адреса ресурсу.", chat_id=message.from_user.id)
...
async def quest8_6(message: types.Message):
    await bot.send_message(text="✅Ні, чим більше доказової бази буде зібрано одразу, тим легше та швидше буде потім пройти процедуру отримання компенсації", chat_id=message.from_user.id)
...
@dp.callback_query_handler(text="🧾Збір  чеків на пошкоджене/зруйноване майно")
...
async def quest9_1(message: types.Message):
    await bot.send_message(text="❗️Якщо відсутні чеки або інші документи, які підтверджують приналежність майна потерпілому та його вартість, можна спробувати скористатися такими доказами:\n"
                                "📷 Фотографії: Збереження фотографій майна до пошкодження може служити доказом його наявності та стану. Якщо у вас є фотографії майна, на яких відображено його цілісність та стан, вони можуть бути використані для документування пошкоджень.\n"
                                "💳 Витяги з банківських виписок: Якщо покупка майна була здійснена за допомогою банківського переказу або оплати з банківської картки, можна намагатися знайти відповідні витяги з банківських виписок, які підтверджують здійснення операції та вартість майна.\n"
                                "👥 Свідчення свідків: Якщо є свідки, які можуть підтвердити приналежність майна потерпілому або його вартість, їхні свідчення можуть бути використані як доказ.", chat_id=message.from_user.id)
...
async def quest9_2(message: types.Message):
    await bot.send_message(text="🧾 Супутніми доводами можуть виступати квитанції на рухоме майно (техніку, речі та інше, що знаходилось в будинку), фото майна до та після руйнування.\n"
                                "🧾 Також необхідно зібрати за наявності будь-які чеки (квитанції, фактури) на пошкоджене чи знищене майно, яке знаходилося в нерухомості, зробити з них копії.", chat_id=message.from_user.id)


@dp.message_handler(text="Які можуть бути переваги зберігання чеків купівлі майна?")
async def quest9_3(message: types.Message):
    await bot.send_message(text="✅Ви зможете підтвердити вартість пошкодженого чи втраченого майна для отримання компенсації", chat_id=message.from_user.id)
...
async def quest9_5(message: types.Message):
    await bot.send_message(text="✅ В деяких випадках електронні копії чеків можуть бути прийняті як доказ при заяві про пошкодження або знищення майна, особливо якщо вони мають дату, підпис або печатку магазину.\n"
                                "❗️ Проте, в інших випадках можуть бути встановлені певні вимоги щодо збереження оригінальних чеків або їх паперових копій.", chat_id=message.from_user.id)


@dp.message_handler(text="🚰Збір інформації про підключення комунальних послуг")
@dp.callback_query_handler(text="🚰Збір інформації про підключення комунальних послуг")
...
async def quest10_1(message: types.Message):
    await bot.send_message(text="✅ Документ, що підтверджує право власності або користування житловою одиницею (наприклад, договір купівлі-продажу, орендний договір).\n"
                                "✅ Заява на підключення комунальних послуг до відповідних підрозділів (наприклад, газової компанії, енергопостачальної компанії, водопостачальної компанії тощо).\n"
                                "✅ Документи, які засвідчують технічну можливість підключення (наприклад, акт технічної можливості на встановлення лічильника).\n"
                                "✅ Інші документи, які можуть бути вимогами конкретної комунальної компанії або організації.", chat_id=message.from_user.id)


@dp.message_handler(text="Які документи слід додати до акту?")
async def quest10_2(message: types.Message):
    await bot.send_message(text="За наявності вузлів розподільного обліку води (квартирних лічильників):\n"
                                "✅ паспорти на лічильники та/або свідоцтво про повірку; \n"
                                "Для власників вузлів розподільного обліку теплової енергії (квартирних лічильників опалення):\n"
                                "✅ акт технічної можливості на встановлення лічильника;\n"
                                "✅ паспорт на встановлений лічильник та акт введення лічильника в експлуатацію;\n"
                                "✅ проєктно-технічну документацію на індивідуальне опалення (якщо воно є);\n"
                                "✅ акти опломбування всіх лічильників.", chat_id=message.from_user.id)
...
async def quest10_3_4(message: types.Message):
    await bot.send_message(text="Якщо документи про підключення комунальних послуг були втрачені, їх можна отримати наступними способами:\n"
                                "📝👨‍🔧 Звернутися до відповідних комунальних компаній або організацій і запитати копії документів.\n"
                                "📞 Зателефонувати до служби підтримки або відділу обслуговування і запросити відновлення документів.\n"
                                "📝 👨‍💼 Звернутися до органів місцевого самоврядування або відповідних відділів з питань комунальних послуг і отримати необхідну інформацію та документи.", chat_id=message.from_user.id)


@dp.message_handler(text="Які комунальні послуги потрібно зазначати в акті?")
async def quest10_5(message: types.Message):
    await bot.send_message(text="⚠️ Газопостачання; \n"
                                "⚡️ Електропостачання;\n"
                                "🚰 Водопостачання; водовідведення;\n"
                                "🌡Теплопостачання.", chat_id=message.from_user.id)
...
async def quest10_6(message: types.Message):
    await bot.send_message(text="✅ Проєктно-технічна документація на індивідуальне опалення;\n"
                                "✅ Паспорти на лічильники та/або свідоцтво про повірку для власників лічильників опалення – акт технічної можливості на встановлення вузла розподільного обліку теплової енергії;\n"
                                "✅ Паспорт на встановлений лічильник опалення;\n"
                                "Акт введення лічильника в експлуатацію;\n"
                                "✅ Проектно-технічну документацію на індивідуальне опалення (якщо воно є);\n"
                                "✅ Акти опломбування всіх лічильників.", chat_id=message.from_user.id)
...
async def quest10_7(message: types.Message):
    await bot.send_message(text="✅ Можна звернутися до відповідних комунальних компаній/організацій, і запитати про можливість надання копій або дублікатів документів.\n"
                                "✅ Також можна звернутися до органів місцевого самоврядування або відділів з питань комунальних послуг і отримати необхідну допомогу у відновленні документів.", chat_id=message.from_user.id)
...
async def quest10_10(message: types.Message):
    await bot.send_message(text="❌Відсутність документів про підключення комунальних послуг може вплинути на процес отримання Акту про пошкоджене майно, оскільки недостатньо доказів та інформації про стан підключених послуг;\n"
                                "❗️ Проте, в такому випадку можна звернутися до відповідних комунальних компаній, органів місцевого самоврядування або відділів з питань комунальних послуг, щоб провести додаткову перевірку та отримати необхідну інформацію та документи, які можуть замінити відсутні документи про підключення комунальних послуг.", chat_id=message.from_user.id)


@dp.message_handler(text="🗃Збереження зібраних матеріалів")
@dp.callback_query_handler(text="🗃Збереження зібраних матеріалів")
...
async def quest11_1(message: types.Message):
    await bot.send_message(text="✅Так, рекомендується зберігати зібрані документи в електронному форматі для забезпечення зручності доступу та запобігання втрати фізичних копій.", chat_id=message.from_user.id)


@dp.message_handler(text="Які документи потрібно сканувати та зберігати?")
async def quest11_2(message: types.Message):
    await bot.send_message(text="✅Потрібно сканувати та зберігати всі зібрані матеріали:\n"
                                "📑Документи;\n"
                                "📷 Фотографії;\n"
                                "📑 Акти;\n"
                                "✉️ Листи тощо.", chat_id=message.from_user.id)


@dp.message_handler(text="На яких пристроях зберігати скани документів?")
async def quest11_3(message: types.Message):
    await bot.send_message(text="Скани документів можна зберігати на різних пристроях, таких як:\n"
                                "🖥💻Комп'ютери, Ноутбуки;\n"
                                "📱Смартфони, Планшети;\n"
                                "💾💿  Зовнішні накопичувачі або спеціальні пристрої для зберігання документів.", chat_id=message.from_user.id)
...
async def quest11_4(message: types.Message):
    await bot.send_message(text="Для зберігання сканів документів в електронному вигляді можна використовувати різні хмарні сховища, такі як:\n"
                                "✅Google Drive, Dropbox, Microsoft OneDrive, iCloud тощо;\n"
                                "❗️Вибір конкретного хмарного сховища залежатиме від ваших вподобань та вимог щодо безпеки та доступності.", chat_id=message.from_user.id)


@dp.message_handler(text="Чи обов’язково відправляти скани документів комусь ще?")
async def quest11_5(message: types.Message):
    await bot.send_message(text="✅Надсилання сканів документів комусь ще є необов'язковим, але може бути корисним для забезпечення додаткової копії та запобігання можливій втраті. Розгляньте можливість надіслати скани родичам або довіреним особам, які можуть бути вам підтримкою.", chat_id=message.from_user.id)
...
async def quest11_7(message: types.Message):
    await bot.send_message(text="✅Рекомендується регулярно перевіряти та оновлювати збережені скани, особливо в разі змін у ваших документах або додавання нових важливих документів, а також робити резервні копії важливих даних.", chat_id=message.from_user.id)


@dp.message_handler(text="👮‍Звернення до НПУ із заявою")
@dp.callback_query_handler(text="👮‍Звернення до НПУ із заявою")
...
async def quest12_2(message: types.Message):
    await bot.send_message(text="📝Заява, в якій слід детально описати ситуацію, порушення законів та звичаїв війни і надати всі відомості про події.\n"
                                "📑Документи, що підтверджують факти порушення, такі як фотографії, відеозаписи, свідчення свідків тощо.", chat_id=message.from_user.id)


@dp.message_handler(text="Для чого потрібен витяг з ЄРДР?")
async def quest12_3(message: types.Message):
    await bot.send_message(text="📄Витяг з Єдиного реєстру досудових розслідувань (ЄРДР) є важливим документом, який надається заявнику. Він містить інформацію про відкрите кримінальне провадження і є підтвердженням того, що ваша заява прийнята та зареєстрована. Витяг з ЄРДР може бути використаний для подальшого взаємодії з поліцією, статусу потерпілого та вирішення питань щодо стягнення спричиненої шкоди.", chat_id=message.from_user.id)


@dp.message_handler(text="👩‍💻Подання онлайн заявок")
@dp.callback_query_handler(text="👩‍💻Подання онлайн заявок")
...
async def quest13_1(message: types.Message):
    await bot.send_message(text="✅Для здійснення повідомлень використовується Єдиний портал державних послуг «Дія». Подати інформаційне повідомлення про зруйноване внаслідок військової агресії рф житло можна онлайн (через мобільний застосунок «Дія» чи через портал державних послуг «Дія» – diia.gov.ua) або офлайн: звернувшись до ЦНАПу чи нотаріуса особисто або через уповноваженого представника.\n"
                                "✅В рамках програми «єВідновлення» можна отримати компенсації на відновлення пошкодженого майна через війну.", chat_id=message.from_user.id)
